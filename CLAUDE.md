...
**NEVER create new template files** when nixifying existing
omarchy configs. Instead, modify the existing file in `config/`
or `default/` to add `@placeholder@` substitutions. This project
patches on top of upstream omarchy — modifying existing files
creates merge conflicts on rebase, which is intentional: it makes
upstream changes visible and easy to review.

Example: To nixify `config/brave-flags.conf`, change:
```
--load-extension=~/.local/share/omarchy/...
```
to:
```
--load-extension=@copyUrlExtension@
```

Then use `pkgs.replaceVars` in the nix module to substitute the
nix store path.
