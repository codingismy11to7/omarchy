...
## Nix Linting
After editing Nix files, run statix to check for linting issues:
```bash
nix run nixpkgs#statix -- check <file.nix>
```
Fix all statix warnings, not just errors.
...
- Use `inherit (pkgs) foo;` rather than `foo = pkgs.foo;` for package assignments
...
- New NixOS sub-modules go in `nix/modules/<name>/core.nix`
- New home-manager modules go in `nix/modules/<name>/home.nix`
...
@jq@ -r '.foo' | @grep@ bar
```
Scripts do NOT include a shebang — `createScript` in `nix/modules/scripts/home.nix` prepends `#!/path/to/bash` automatically at build time.
