...
mkIf vt.enable {
  ...
  xdg.configFile."voxtype/config.toml".source =
    pkgs.replaceVars (path { path = ../../../default/voxtype/config.toml; })
      {
        inherit whisperModel;
        audioFeedbackEnabled = if feedback != null then "true" else "false";
        audioFeedbackTheme = if feedback != null then feedback.theme else "default";
        audioFeedbackVolume = toString (if feedback != null then feedback.volume else 0.7);
      };
}
